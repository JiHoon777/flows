...
export const SiteLeftExplorerFlowRow = memo(({ flow }: { flow: Flow }) => {
  ...
  const isViewing = flow.flowId === flowId
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <span className={'text-sm'}>{flow.data?.title ?? '-'}</span>
        ...
        {isOpen && (
          ...
          >
            {flow.childFlowIds?.map((cf) => {
              const found = getFlowById(cf)
              return found ? (
                <SiteLeftExplorerFlowRow key={found.flowId} flow={found} />
              ) : null
            })}
            {flow.childNodeIds?.map((cn) => {
              const found = getNodeById(cn)
              return found?.type === 'note' ? (
                <SiteLeftExplorerNodeRow key={found.nodeId} node={found} />
              ) : null
            })}
          </motion.div>
        )}
